...
export default function PricingPage({ onBack, onNavigate }) {
  ...
  useEffect(() => {
    apiClient("/api/billing/plans")
      ...
      .catch(() => setStatus("Using local pricing until backend billing config is deployed."));
  }, []);
  ...
  const selectedMethod = [...(methods?.india || []), ...(methods?.global || [])].find((m) => m.id === paymentMethod);
  const displayAmount = paymentMethod === "paypal" || paymentMethod === "stripe" ? `$${selected.usdAmount || 2}` : `₹${selected.amount}`;

  const createIntent = async () => {
    setError("");
    setStatus("Creating secure payment intent...");
    try {
      const data = await apiClient("/api/billing/payment-intents", { method: "POST", body: JSON.stringify({ plan: selectedPlan, provider: paymentMethod }) });
  ...
  const submitRequest = async (event) => {
    ...
    try {
      await apiClient("/api/billing/upi-request", {
        ...
        body: JSON.stringify({
          plan: selectedPlan,
          amount: paymentMethod === "paypal" || paymentMethod === "stripe" ? (selected.usdAmount || 2) : selected.amount,
          paymentMethod,
          upiId: form.upiId,
      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_1.2fr]">
        <section className="rounded-3xl border border-white/10 bg-white/[0.04] p-5">
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            {[...(methods?.india || []), ...(methods?.global || [])].map((method) => (
          ...
          <div className="mt-5 rounded-2xl border border-dashed border-amber-300/30 bg-black/20 p-4">
            ...
            {paymentMethod === "bank" && <div className="mt-3 space-y-2 rounded-xl bg-white/5 p-3 text-sm text-slate-300"><p>Account: {selectedMethod?.bank?.accountName}</p><p>No: {selectedMethod?.bank?.accountNumber}</p><p>IFSC: {selectedMethod?.bank?.ifsc}</p><p>Bank: {selectedMethod?.bank?.bankName}</p></div>}
            {(paymentMethod === "paypal" || paymentMethod === "stripe") && <button onClick={createIntent} className="mt-4 w-full rounded-xl bg-[#81b64c] px-4 py-3 font-black text-black">Create intent / open checkout</button>}
