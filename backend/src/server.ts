...
initMonitoring();
try { validateEnv(); } catch (error) { fatalConfigError(error?.message || "Invalid environment configuration"); }
...
import { isValidMove as validateMove, applyMove, opponent, } from "./chessUtils";
import { createInitialGameState, isPlayableStatus, toGameResult, } from "./gameState";
...
import openingRoutes from "./routes/openings";
...
import { updatePlayerStats } from "./utils/elo";
...
async function shutdown(signal) {
  logger.info(`${signal} received. Closing HTTP server and Prisma connection.`);
  server.close(async () => {
    try { await disconnectPrisma(); } catch (error) { logger.error("Prisma disconnect failed", error); }
    process.exit(0);
  });
}
