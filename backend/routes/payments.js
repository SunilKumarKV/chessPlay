...
async function couponDiscount(code, plan) {
  ...
  if (!normalized) return { code: "", discountPercent: 0 };
  const coupon = await Coupon.findOne({ code: normalized, active: true });
  if (!coupon) return { code: normalized, discountPercent: 0, invalid: true };
  if (coupon.expiresAt && coupon.expiresAt.getTime() < Date.now()) return { code: normalized, discountPercent: 0, invalid: true };
  if (coupon.maxRedemptions && coupon.redeemedCount >= coupon.maxRedemptions) return { code: normalized, discountPercent: 0, invalid: true };
  if (coupon.plans?.length && !coupon.plans.includes(plan)) return { code: normalized, discountPercent: 0, invalid: true };
  return { code: normalized, discountPercent: coupon.discountPercent || 0 };
}
...
router.post("/create-order", auth, paymentLimiter, async (req, res) => {
  try {
    ...
    const amount = Math.max(1, Math.round(config.amountInr * (1 - coupon.discountPercent / 100)));
...
router.post("/webhook", express.raw({ type: "application/json" }), async (req, res) => {
  try {
    ...
    const webhookEventId = String(event?.payload?.payment?.entity?.id || event?.event || "");
    if (webhookEventId && await Payment.exists({ webhookEventId })) return res.json({ received: true, duplicate: true });
    await Payment.create({
      provider: "razorpay",
...
router.post("/coupon/validate", paymentLimiter, async (req, res) => {
  ...
  res.json({ valid: Boolean(result.discountPercent), ...result });
});
